...
const GenerateCustomTrekRouteInputSchema = z.object({
  destinationName: z.string().describe('The general area, starting point, or a known trek name to base the custom route on (e.g., "Manali region", "Sankri for Kedarkantha", "Roopkund area", "Valley of Flowers"). This provides context for the AI.'),
  durationDays: z.number().min(1).max(30).describe('Desired duration of the trek in days. The AI may adjust this slightly for a feasible plan.'),
  difficulty: z.enum(['Easy', 'Moderate', 'Challenging', 'Expert']).describe('Preferred difficulty level of the trek.'),
  specificInterests: z.string().optional().describe('Optional specific interests or preferences to guide route generation, e.g., "alpine lakes", "panoramic views", "less crowded trails", "photography focused", "cultural immersion", "want to see specific peaks like Trishul".'),
});
export type GenerateCustomTrekRouteInput = z.infer<typeof GenerateCustomTrekRouteInputSchema>;

const DailyPlanSchema = z.object({
  day: z.number().describe('The day number of the itinerary (e.g., 1, 2, 3).'),
  title: z.string().describe('A short, engaging title for the day, e.g., "Arrival in Leh & Acclimatization" or "Trek from Govindghat to Ghangaria".'),
  routeDescription: z.string().describe("Detailed plan for the day, including start and end points, key villages or landmarks passed, and approximate trekking time or distance. Example: 'Trek from Sari village to Deoriatal (2.5 km, approx 2-3 hours). Gradual ascent through rhododendron forests.'"),
  altitude: z.string().optional().describe('Approximate altitude of the campsite or key points reached during the day (e.g., "Deoriatal: 2,438m").'),
  highlights: z.array(z.string()).min(1).describe('List at least one key highlight, activity, or point of interest for the day (e.g., "Reflection of Chaukhamba peaks in Deoriatal", "Visit Hemkund Sahib Gurudwara").'),
});

const GenerateCustomTrekRouteOutputSchema = z.object({
  routeName: z.string().describe('A creative and suggestive name for the generated custom trek route (e.g., "The Celestial Lakes Circuit" or "Hidden Valleys of Garhwal").'),
  overview: z.string().describe('A brief (2-3 sentences) compelling overview of the entire trek route, highlighting its main appeal.'),
  suggestedDurationDays: z.number().describe('The AI suggested duration for the trek in days, which might be adjusted from the user input for a more practical plan.'),
  difficultyRating: z.string().describe('The AI\'s assessed difficulty level for the generated route (e.g., "Moderate with some challenging sections").'),
  bestSeason: z.string().optional().describe('Recommended best season or months to undertake this specific trek (e.g., "May to June and September to October").'),
  dailyItinerary: z.array(DailyPlanSchema).min(1).describe('A day-by-day itinerary. The number of days should match suggestedDurationDays.'),
  preparationNotes: z.array(z.string()).optional().describe('Important notes for preparation, such as permit requirements, fitness level advised, acclimatization guidelines, or general travel advice for the region. Each note as a separate string.'),
  gearSuggestions: z.array(z.string()).optional().describe('List of 3-5 key gear items specifically recommended for this trek (e.g., "Sturdy trekking shoes with ankle support", "Sub-zero sleeping bag", "Trekking poles"). Each item as a separate string.'),
});
export type GenerateCustomTrekRouteOutput = z.infer<typeof GenerateCustomTrekRouteOutputSchema>;
...
const generateCustomTrekRouteFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    if (!output) {
      throw new Error('AI failed to generate a trek route. The output was empty.');
    }
    ...
    if (output.dailyItinerary.length !== output.suggestedDurationDays) {
        ...
        console.warn(`AI Warning: Mismatch between suggestedDurationDays (${output.suggestedDurationDays}) and actual itinerary days (${output.dailyItinerary.length}). Adjusting suggestedDurationDays.`);
        ...
        if (output.dailyItinerary.length === 0 && input.durationDays > 0) {
            ...
            throw new Error('AI generated an empty daily itinerary. Please try refining your request.');
        }
    }
    return output;
  }
