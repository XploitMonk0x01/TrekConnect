...
import type { Story, CreateStoryInput, UserProfile } from '@/lib/types'; // UserProfile for author info
...
function mapDocToStory(doc: WithId<StoryDocument>): Story { // UserProfile arg removed
  return {
    id: doc._id.toString(),
...
export async function getStoriesByUser(userId: string): Promise<Story[]> {
  try {
    ...
    const storyDocs = await collection.find({ userId: userId }).sort({ createdAt: -1 }).toArray();
    return storyDocs.map(mapDocToStory);
  } catch (error) {
...
export async function updateStory(storyId: string, updates: Partial<Story>, authorId: string): Promise<Story | null> {
  try {
    ...
    const existingStory = await collection.findOne({ _id: new ObjectId(storyId) });
    if (!existingStory || existingStory.userId !== authorId) {
      throw new Error('Story not found or user not authorized to update.');
    }

    const { id, userId, userName, userAvatarUrl, createdAt, ...restUpdates } = updates; // Exclude fields not directly updatable this way
    const updateData: Partial<Omit<StoryDocument, '_id' | 'userId' | 'userName' | 'userAvatarUrl' | 'createdAt'>> = {
        ...restUpdates,
        updatedAt: new Date(),
    };
...
export async function likeStory(storyId: string, likingUserId: string): Promise<void> {
  try {
    const db: Db = await getDb();
    const collection = db.collection<StoryDocument>('stories');
    ...
    console.error(`Error liking story ${storyId} by user ${likingUserId}:`, error);
    throw error;
  }
}

export async function unlikeStory(storyId: string, unlikingUserId: string): Promise<void> {
  try {
    const db: Db = await getDb();
    const collection = db.collection<StoryDocument>('stories');
    await collection.updateOne(
    ...
    console.error(`Error unliking story ${storyId} by user ${unlikingUserId}:`, error);
    throw error;
  }
}
...
export async function deleteStory(storyId: string, authorId: string): Promise<void> {
  try {
    ...
    const result = await collection.deleteOne({ _id: new ObjectId(storyId), userId: authorId });
