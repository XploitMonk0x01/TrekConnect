...
function mapDocToPhoto(doc: WithId<PhotoDocument>): Photo {
  // UserProfile arg removed, assume denormalized data
  return {
    id: doc._id.toString(),
...
export async function getPhotosByUser(userId: string): Promise<Photo[]> {
  try {
    ...
    const photoDocs = await collection
      .find({ userId: userId })
      .sort({ uploadedAt: -1 })
      .toArray()
    return photoDocs.map(mapDocToPhoto)
...
export async function likePhoto(
  ...
  try {
    ...
    console.error(
      `Error liking photo ${photoId} by user ${likingUserId}:`,
      error
    )
    throw error
  }
...
export async function unlikePhoto(
  ...
  try {
    ...
    console.error(
      `Error unliking photo ${photoId} by user ${unlikingUserId}:`,
      error
    )
    throw error
  }
