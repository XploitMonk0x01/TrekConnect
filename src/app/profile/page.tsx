...
const WishlistAITags = [
  'kashmir lakes',
  'uttarakhand roopkund',
  'himalayan peaks',
  'alpine meadow',
]
const HistoryAITags = [
  'himachal hampta',
  'uttarakhand kedarkantha',
  'himachal triund',
  'mountain pass',
]
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 text-sm mt-4">
            <div className="flex items-center p-3 bg-background rounded-lg border">
              ...
              <div>
                ...
                {profile.travelPreferences?.style
                  ? `, ${profile.travelPreferences.style}`
                  : ''}
              </div>
            </div>
      ...
      <Tabs defaultValue="wishlist">
        <TabsList className="grid w-full grid-cols-2 md:w-1/2 lg:w-1/3 mx-auto">
          <TabsTrigger value="wishlist">
            ...
            Wishlist ({profile.wishlistDestinations?.length || 0})
          </TabsTrigger>
          <TabsTrigger value="history">
            <CheckCircle className="mr-2 h-4 w-4" />
            Travel History ({profile.travelHistory?.length || 0})
          </TabsTrigger>
        </TabsList>
        <TabsContent value="wishlist" className="mt-6">
          <Card>
            ...
            <CardContent>
              {profile.wishlistDestinations &&
              profile.wishlistDestinations.length > 0 ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                  {profile.wishlistDestinations.map((dest, index) => (
                    <DestinationItemCard
                      key={dest.id}
                      destination={dest}
                      aiHint={WishlistAITags[index % WishlistAITags.length]}
                    />
                  ))}
                </div>
              ) : (
          ...
          <Card>
            ...
            <CardContent>
              {profile.travelHistory && profile.travelHistory.length > 0 ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                  {profile.travelHistory.map((dest, index) => (
                    <DestinationItemCard
                      key={dest.id}
                      destination={dest}
                      aiHint={HistoryAITags[index % HistoryAITags.length]}
                    />
                  ))}
                </div>
              ) : (
