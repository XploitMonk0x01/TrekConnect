...
if (!JWT_SECRET) {
  throw new Error('JWT_SECRET is not defined in the environment variables for token validation.');
}
...
export async function GET(request: Request) {
  try {
    ...
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
      return NextResponse.json({ error: 'No token provided or malformed header' }, { status: 401 });
    }
    
    const token = authHeader.split(' ')[1];
    if (!token) {
      return NextResponse.json({ error: 'No token found after Bearer' }, { status: 401 });
    }

    const decoded = verify(token, JWT_SECRET) as JwtPayload;
    
    if (!decoded || !decoded.id || !ObjectId.isValid(decoded.id)) {
      return NextResponse.json({ error: 'Invalid token payload or user ID' }, { status: 401 });
    }
    ...
    if (!userDoc) {
      return NextResponse.json({ error: 'User not found for token ID' }, { status: 404 });
    }
    ...
    if (error instanceof JsonWebTokenError) {
      return NextResponse.json({ error: 'Invalid token signature or structure' }, { status: 401 });
    }
    return NextResponse.json({ error: 'Internal server error during token validation' }, { status: 500 });
  }
}
