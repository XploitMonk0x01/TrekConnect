...
export default function ExploreClientComponent({ initialDestinations }: ExploreClientComponentProps) {
  ...
  const handleApplyAIFilter = async () => {
    if (!aiFilterQuery.trim()) {
      toast({ variant: "destructive", title: "Empty Filter", description: "Please enter what you're looking for." });
      return;
    }
    ...
    try {
      ...
      const result = await filterDestinations({ filterQuery: aiFilterQuery, destinationsToFilter });
      setAiFilteredDestinationIds(result.relevantDestinationIds);
      setAiFilterExplanation(result.aiExplanation || "AI has filtered the destinations.");
      toast({ title: "AI Filter Applied", description: result.aiExplanation || "Destinations have been filtered." });
    } catch (error: any) {
  ...
  const displayedDestinations = useMemo(() => {
    let currentList = destinations.filter(destination =>
  ...
  const handleToggleWishlist = async (destinationId: string, destinationName: string) => {
    if (!currentUser) {
      toast({ title: "Authentication Required", description: "Please sign in to add items to your wishlist.", action: <Button onClick={() => router.push('/auth/signin?redirect=/explore')}>Sign In</Button>, });
      return;
    }
    ...
    const newWishlist = isWishlisted ? currentWishlist.filter(name => name !== destinationName) : [...currentWishlist, destinationName];
    try {
      const updatedUser = await updateUserProfile(currentUser.id, { wishlistDestinations: newWishlist });
      if (updatedUser) { updateUserInContext(updatedUser); toast({ title: isWishlisted ? "Removed from Wishlist" : "Added to Wishlist", description: `${destinationName} has been ${isWishlisted ? 'removed from' : 'added to'} your wishlist.`, });
      } else { throw new Error("Failed to update wishlist on server."); }
    } catch (error) { console.error("Error updating wishlist:", error); toast({ variant: "destructive", title: "Wishlist Update Failed", description: "Could not update your wishlist.", });
    } finally { setWishlistProcessing(prev => ({ ...prev, [destinationId]: false })); }
  };
  ...
  const isLoadingInitialMedia = destinations.some(d => d.isLoadingImage || d.isLoadingYouTubeVideoId);
    ...
    <div className="space-y-8">
      <Card className="shadow-lg">
        <CardHeader>
          ...
          <CardDescription>Find your next Himalayan adventure. Search for treks or use our AI-powered filter.</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="flex flex-col gap-4 mb-6">
            <div className="relative flex-grow">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" />
              <Input type="search" placeholder="Search treks (e.g., Roopkund, Ladakh, Winter trek)" className="pl-10" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
            </div>
            <div className="flex flex-col sm:flex-row gap-2">
              <Input type="text" placeholder="AI Filter: e.g., 'easy treks in Uttarakhand for beginners'" className="flex-grow" value={aiFilterQuery} onChange={(e) => setAiFilterQuery(e.target.value)} disabled={isFilteringWithAI} />
              <Button onClick={handleApplyAIFilter} disabled={isFilteringWithAI || !aiFilterQuery.trim()} className="bg-accent hover:bg-accent/90">
                {isFilteringWithAI ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Wand2 className="mr-2 h-4 w-4" />} Apply AI Filter
              </Button>
              {aiFilteredDestinationIds && (
                <Button variant="outline" onClick={handleClearAIFilter} disabled={isFilteringWithAI}>
      ...
      <Card>
        ...
        <CardContent>
          {mapUrl ? ( <iframe width="100%" height="400" frameBorder="0" scrolling="no" marginHeight={0} marginWidth={0} src={mapUrl} className="rounded-lg border" title="Interactive Map of Treks" loading="lazy"></iframe> ) : 
          ( <div className="h-96 w-full bg-muted rounded-lg flex items-center justify-center text-muted-foreground"> <Loader2 className="h-8 w-8 animate-spin mr-2" /> Loading map... </div> )}
        </CardContent>
      </Card>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[...Array(3)].map((_, index) => (
            <Card key={`skeleton-${index}`} className="overflow-hidden flex flex-col">
              ...
              <CardFooter className="p-4 border-t grid grid-cols-3 gap-2 items-center"><Skeleton className="h-5 w-12" /><Skeleton className="h-8 w-full" /><Skeleton className="h-8 w-full" /></CardFooter>
            </Card>
          ))}
        </div>
      )}

      {!isLoadingInitialMedia && !isFilteringWithAI && displayedDestinations.length === 0 && (
         <Card>
            <CardContent className="p-6 text-center">
                <MapPin className="h-12 w-12 mx-auto text-muted-foreground mb-4" />
                <h3 className="text-xl font-semibold">No Treks Found</h3>
                <p className="text-muted-foreground">
                    {searchQuery || aiFilterQuery ? "Try adjusting your search or AI filter query." : "There are no treks available at the moment."}
                </p>
            </CardContent>
        </Card>
      )}
          ...
          (destination.isLoadingImage || destination.isLoadingYouTubeVideoId) && !isFilteringWithAI ? ( 
            ...
            <Card key={destination.id} className="overflow-hidden hover:shadow-xl transition-shadow duration-300 flex flex-col">
              <CardHeader className="p-0 relative h-48">
                <Image src={destination.fetchedImageUrl || PLACEHOLDER_IMAGE_URL(600,400)} alt={destination.name} layout="fill" objectFit="cover" data-ai-hint={destination.aiHint || destination.name.toLowerCase().split(' ').slice(0,2).join(' ')} onError={(e) => { (e.target as HTMLImageElement).src = PLACEHOLDER_IMAGE_URL(600,400); }} />
                 <Button size="icon" variant="ghost" className="absolute top-2 right-2 bg-black/30 hover:bg-black/50 text-white hover:text-pink-400 rounded-full" onClick={() => handleToggleWishlist(destination.id, destination.name)} disabled={wishlistProcessing[destination.id] || authIsLoading} aria-label={isDestinationInWishlist(destination.name) ? "Remove from wishlist" : "Add to wishlist"}>
                  {wishlistProcessing[destination.id] ? <Loader2 className="h-5 w-5 animate-spin" /> : <Heart className={`h-5 w-5 ${isDestinationInWishlist(destination.name) ? 'fill-pink-500 text-pink-500' : 'text-white'}`} />}
                 </Button>
              </CardHeader>
