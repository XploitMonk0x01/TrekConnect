...
const initialFeaturesData: Omit<Feature, 'imageUrl'>[] = [
  { title: "Explore Destinations", description: "Discover breathtaking new places.", icon: Compass, href: "/explore", queryHint: "himalayan peaks", imageHint: "himalayan peaks" },
  { title: "ConnectSphere", description: "Find like-minded travel companions.", icon: Users, href: "/connect", queryHint: "happy hikers", imageHint: "happy hikers" },
  { title: "Smart Recommendations", description: "AI-powered travel suggestions.", icon: Wand2, href: "/recommendations", queryHint: "ai travel map", imageHint: "ai travel map" },
  { title: "Photo Feed", description: "Share and view travel photos.", icon: ImageIcon, href: "/feed", queryHint: "travel photography", imageHint: "travel photography" },
  { title: "Travel Stories", description: "Read and write inspiring tales.", icon: BookOpen, href: "/stories", queryHint: "adventure journal", imageHint: "adventure journal" },
];

export default function DashboardPage() {
  const [heroImageUrl, setHeroImageUrl] = useState<string>(PLACEHOLDER_IMAGE_URL(1200, 400));
  const [features, setFeatures] = useState<Feature[]>(
    initialFeaturesData.map(f => ({ ...f, imageUrl: PLACEHOLDER_IMAGE_URL(400, 200) }))
  );
  ...
  useEffect(() => {
    const fetchImages = async () => {
      ...
      try {
        ...
        const fetchedHeroImageUrl = await searchPexelsImage(heroQuery, 1200, 400);
        setHeroImageUrl(fetchedHeroImageUrl);

        const fetchedFeatures = await Promise.all(
          initialFeaturesData.map(async (feature) => {
            const imageUrl = await searchPexelsImage(feature.queryHint, 400, 200);
      ...
      <Card className="shadow-lg">
        ...
        <CardContent className="relative h-64 sm:h-80 md:h-96 rounded-b-lg overflow-hidden">
          <Image
            ...
            onError={(e) => { (e.target as HTMLImageElement).src = PLACEHOLDER_IMAGE_URL(1200, 400); }}
          />
          ...
          <Card key={feature.title} className="flex flex-col hover:shadow-xl transition-shadow duration-300">
            ...
            <CardContent className="flex-1 relative h-40 rounded-md overflow-hidden mb-4 mx-6">
               <Image
                ...
                onError={(e) => { (e.target as HTMLImageElement).src = PLACEHOLDER_IMAGE_URL(400, 200); }}
              />
            </CardContent>
